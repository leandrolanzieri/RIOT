...
static void test_gcoap__client_get_req(void)
{
...
#endif
    memcpy(&pdu_data[hdr_fixed_len + CONFIG_GCOAP_TOKENLEN], &options[0], options_len);

    len = gcoap_request(&pdu, &buf[0], CONFIG_GCOAP_PDU_BUF_SIZE, COAP_METHOD_GET,
                                                           &path[0]);
    ...
    TEST_ASSERT_EQUAL_INT(hdr_fixed_len + CONFIG_GCOAP_TOKENLEN, coap_get_total_hdr_len(&pdu));
...
static void test_gcoap__client_get_path_defer(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(len, sizeof(coap_hdr_t) + CONFIG_GCOAP_TOKENLEN + optlen);
