...
`draft-moran-suit-manifest-04 <https://datatracker.ietf.org/doc/draft-moran-suit-manifest/04/>`_.
...
* `Setup <#Setup>`_
  ...
  * `Signing key management <#Key-management>`_
  * `Setup a wired device using ethos <#Setup-a-wired-device-using-ethos>`_

    * `Provision the device <#Provision-the-device>`_
    * `Configure the network <#Configure-the-network>`_

  * `Alternative: Setup a wireless device behind a border router <#Setup-a-wireless-device-behind-a-border-router>`_

    * `Provision the wireless device <#Provision-the-wireless-device>`_
    * `Configure the wireless network <#Configure-the-wireless-network>`_

  * `Start aiocoap fileserver <#start-aiocoap-fileserver>`_

* `Perform an update <#Perform-an-update>`_

  * `Build and publish the firmware update <#Build-and-publish-the-firmware-update>`_
  * `Notify an update to the device <#Norify-an-update-to-the-device>`_

* `Detailed explanation <#Detailed-explanation>`_
* `Automatic test <#test>`_\ [test]
...
* 
  ...
  See the `aiocoap installation instructions <https://aiocoap.readthedocs.io/en/latest/installation.html>`_
  for more details.
...
* 
  ...
  See `update <#Perform-an-update>`_ for more information.
...
`SUIT detailed explanation <#Detailed-explanation>`_ for details.
...
`gnrc_border_router <https://github.com/RIOT-OS/RIOT/tree/master/examples/gnrc_border_router>`_
application on it:
...
First un-comment L28 in the application `Makefile <Makefile>`_ so ``gnrc_netdev_default`` is included in the build.
...
* 
  ensure the `prerequisites <#Prerequisites>`_ are installed
...
* `set up the network <#Configure-the-network>`_ (in another shell):
