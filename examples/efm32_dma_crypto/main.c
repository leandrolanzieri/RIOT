...
static uint8_t plain_0[] = {
    0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF,
    0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7
};
// static uint8_t plain_1[] = {
//     0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF,
//     0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
//     0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF,
//     0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7
// };
static uint8_t cipher_0[] = {
    0x37, 0x29, 0xa3, 0x6c, 0xaf, 0xe9, 0x84, 0xff,
    0x46, 0x22, 0x70, 0x42, 0xee, 0x24, 0x83, 0xf6
};
// static uint8_t cipher_1[] = {
//     0x37, 0x29, 0xa3, 0x6c, 0xaf, 0xe9, 0x84, 0xff,
//     0x46, 0x22, 0x70, 0x42, 0xee, 0x24, 0x83, 0xf6,
//     0x37, 0x29, 0xa3, 0x6c, 0xaf, 0xe9, 0x84, 0xff,
//     0x46, 0x22, 0x70, 0x42, 0xee, 0x24, 0x83, 0xf6
...
static uint8_t cipherData[AES_SIZE_BYTES];
...
static void init_crypto_aes_128_encrypt(crypto_device_t* crypto)
{
    ...
    crypto->dev->SEQCTRL |= 16; /* NOTE: we want to handle 16 bytes */
...
int main(void)
{
    ...
    if (IS_ACTIVE(CONFIG_AES_WITH_DMA)) {
        ...
        init_crypto_write_dma_channel(crypto, plain_0);
        init_crypto_aes_128_encrypt(crypto);

        /* wait for instruction, sequence and DMA completion */
        while ((crypto->dev->STATUS &(CRYPTO_STATUS_INSTRRUNNING | CRYPTO_STATUS_SEQRUNNING | CRYPTO_STATUS_DMAACTIVE)) != 0) { }
    }
    else {
        ...
        for (int i = 0; i < 16; i++) {
            crypto->dev->DATA0BYTE = plain_0[i];
        }
        ...
        for (unsigned i = 0; i < AES_SIZE_BYTES; i++) {
    ...
    if (is_result_expected(cipherData, cipher_0, AES_SIZE_BYTES)) {
            puts("=> CORRECT");
    }
    else {
        puts("=> INCORRECT!");
