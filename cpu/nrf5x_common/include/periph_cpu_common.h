...
typedef struct {
     ...
     * further callback happens until it is back within the range. Users can reset the counter from
     * the callback using @ref qdec_read_and_reset to get a callback every at least max counts.
     * Setting this to 0 will disable interrupts. Set this value to 1 to get a callback on every
     * step (with the caveats noted below). A value of 4 is useful with many rotary knobs.
     *
     ...
     *         already be beyond +callback_threshold or -callback_threshold. The upper limit of this
     *         deviation is one report period's worth of steps. Since this interrupt will never
     *         reset the counter, the user can read it via @ref qdec_read_and_reset. To mitigate
     *         this behaviour, use `QDEC_REPORTPER_REPORTPER_1Smpl` as a report period, which will
     *         call the callback exactly at +callback_threshold or -callback_threshold, at the cost
     *         of up to one interrupt per sample while moving.
     */
