...
#if CONFIG_GNRC_IPV6_NIB_CONF_DNS
...
#if CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT
static gnrc_pktqueue_t _queue_pool[CONFIG_GNRC_IPV6_NIB_NUMOF];
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT */
...
#if CONFIG_GNRC_IPV6_NIB_CONF_SLAAC || CONFIG_GNRC_IPV6_NIB_CONF_6LN
    /* TODO: set differently dependent on CONFIG_GNRC_IPV6_NIB_CONF_SLAAC if
     * alternatives exist */
    netif->ipv6.aac_mode = GNRC_NETIF_AAC_AUTO;
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_SLAAC || CONFIG_GNRC_IPV6_NIB_CONF_6LN */
...
#if CONFIG_GNRC_IPV6_NIB_CONF_DC
                _nib_dc_add(&route.next_hop, netif->pid, dst);
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_DC */
            }
            else {
                              ...
                              const icmpv6_hdr_t *icmpv6, size_t icmpv6_len)
{
    ...
    switch (icmpv6->type) {
        ...
        case ICMPV6_REDIRECT:
            /* TODO */
            break;
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_REDIRECT */
#if CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_DAD
        case ICMPV6_DAR:
            /* TODO */
            break;
        case ICMPV6_DAC:
            /* TODO */
            break;
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_DAD */
    }
    mutex_unlock(&_nib_mutex);
    gnrc_netif_release(netif);
}
...
void gnrc_ipv6_nib_handle_timer_event(void *ctx, uint16_t type)
{
    ...
    switch (type) {
        ...
        case GNRC_IPV6_NIB_ADDR_REG_TIMEOUT:
            _nib_nc_remove(ctx);
            break;
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_6LR */
#if CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_P6C
        case GNRC_IPV6_NIB_ABR_TIMEOUT:
            _nib_abr_remove(&((_nib_abr_entry_t *)ctx)->addr);
            break;
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_P6C */
...
#else   /* CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_P6C */
static uint32_t _handle_pio(gnrc_netif_t *netif, const icmpv6_hdr_t *icmpv6,
                            const ndp_opt_pi_t *pio);
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_MULTIHOP_P6C */
/** @} */
    ...
    if (rtr_adv->reach_time.u32 != 0) {
        ...
        if (reach_time != netif->ipv6.reach_time_base) {
            _evtimer_add(netif, GNRC_IPV6_NIB_RECALC_REACH_TIME,
                         ...
                         CONFIG_GNRC_IPV6_NIB_CONF_REACH_TIME_RESET);
...
#else   /* CONFIG_GNRC_IPV6_NIB_CONF_6LR */
#define sl2ao   (NULL)
#define aro     (NULL)
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_6LR */
...
#elif CONFIG_GNRC_IPV6_NIB_CONF_6LN
        (void)aro_status;
#endif
    }
}
...
#else   /* CONFIG_GNRC_IPV6_NIB_CONF_ARSM */
    if (entry != NULL) {
        DEBUG("nib: resolve address %s%%%u from neighbor cache\n",
              ipv6_addr_to_str(addr_str, &entry->ipv6, sizeof(addr_str)),
              _nib_onl_get_if(entry));
...
#else   /* CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT */
            gnrc_icmpv6_error_dst_unr_send(ICMPV6_ERROR_DST_UNR_ADDR,
                                           pkt);
            gnrc_pktbuf_release_error(pkt, EHOSTUNREACH);
#endif  /* CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT */
...
void _handle_search_rtr(gnrc_netif_t *netif)
{
#if !CONFIG_GNRC_IPV6_NIB_CONF_NO_RTR_SOL
    gnrc_netif_acquire(netif);
...
#else
    (void)netif;
#endif /* !CONFIG_GNRC_IPV6_NIB_CONF_NO_RTR_SOL */
}
