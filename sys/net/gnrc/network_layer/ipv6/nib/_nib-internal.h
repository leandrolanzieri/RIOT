...
typedef struct _nib_onl_entry {
...
#if CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT || defined(DOXYGEN)
    /**
     * @brief   queue for packets currently in address resolution
     *
     * @note    Only available if @ref CONFIG_GNRC_IPV6_NIB_CONF_QUEUE_PKT != 0.
     */
     ...
     *
     * @note    Only available if @ref CONFIG_GNRC_IPV6_NIB_CONF_6LR != 0.
     */
    eui64_t eui64;
#endif
#if CONFIG_GNRC_IPV6_NIB_CONF_ARSM || defined(DOXYGEN)
    /**
     * @brief   Link-layer address of _nib_onl_entry_t::next_hop
     *
     * @note    Only available if @ref CONFIG_GNRC_IPV6_NIB_CONF_ARSM != 0.
     */
     ...
     *          @ref CONFIG_GNRC_IPV6_NIB_CONF_ARSM == 0, since 6Lo address
     *          registration uses it to time the sending of neighbor
     *          solicitations.
     */
...
#ifdef CONFIG_GNRC_IPV6_NIB_CONF_ROUTER
    /**
...
#if CONFIG_GNRC_IPV6_NIB_CONF_DC || DOXYGEN
/**
 ...
 * @param[in] iface     The interface to the destination.
 *
 * @note    Only available if @ref CONFIG_GNRC_IPV6_NIB_CONF_DC.
 *
 * @return  A new or existing off-link entry with _nib_offl_entry_t::pfx set to
 *          @p pfx.
 * @return  NULL, if no space is left.
...
#if CONFIG_GNRC_IPV6_NIB_CONF_ROUTER || DOXYGEN
/**
 ...
 * @param[in] pfx_len   The length in bits of @p pfx in bits.
 *
 * @note    Only available if @ref CONFIG_GNRC_IPV6_NIB_CONF_ROUTER.
 *
 * @return  A new or existing off-link entry with _nib_offl_entry_t::pfx set to
 *          @p pfx.
 * @return  NULL, if no space is left.
