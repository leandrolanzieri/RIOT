...
enum {
    ...
    CORD_LC_NORSC     = -4,     /**< lookup interface for not found */
};

#define CORD_LC_ATTR_LEN        (NANOCOAP_URI_MAX)
...
/* Use the same struct for endpoint and resource for now.
...
struct cord_lc_result {
    ...
    clif_param_t *params;       /** Array of Link Format parameters */
    size_t max_params;          /** Max parameters at @p params */
};
...
typedef struct cord_lc_filter {
    clif_param_t *array;                /**< Array of filter(s) */
