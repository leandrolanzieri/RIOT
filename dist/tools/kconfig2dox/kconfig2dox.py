...
def write_define(dox, node):
    if node.prompt:
        sym = node.item
        content = "/**"
        write_content(dox, content, 0)
        content = " * @brief {}".format(node.prompt[0])
        write_content(dox, content, 0)
        content = " */"
        write_content(dox, content, 0)
        content = "#define {}".format(sym.name)
        write_content(dox, content, 0)

def write_symbol_start2(dox, node, level, state):
    sym = node.item
...
def should_write_define(node, state):
    sym = node.item
    if sym.type != 'UNKNOWN' and sym.nodes:
...
def write_node2(dox, node, level, state):
    needs_closure = False
    if isinstance(node.item, kconfiglib.Symbol) and should_write_define(node, state):
        write_define(dox, node)
#    if isinstance(node.item, kconfiglib.Symbol):
#        needs_closure = write_symbol_start2(dox, node, level, state)
...
#        write_dl_start(dox, level + 1)
        entry = node.list
        while (entry):
            write_node2(dox, entry, level + 2, state)
            entry = entry.next
...
def main():
    ...
    header = ("/**\n"
              ...
              " * @{\n"
              " */\n")
    args.dox.write(header)
    write_node2(args.dox, kconf.top_node, 1, state)
    args.dox.write("/** @} */")
