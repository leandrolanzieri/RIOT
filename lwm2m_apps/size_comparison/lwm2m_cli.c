...
#define BASE_OBJ_NUM (5)

#define OBJ_COUNT (BASE_OBJ_NUM + 3 * IS_ACTIVE(CONFIG_LWM2M_CLIENT_C2C))
...
void lwm2m_cli_init(void)
{
    ...
    obj_list[4] = lwm2m_object_light_control_get();
    ...
    res = lwm2m_object_light_control_instance_create(obj_list[4], 0, &light_args);
    ...
    obj_list[5] = lwm2m_object_client_get();
    obj_list[6] = lwm2m_object_client_security_get();
    obj_list[7] = lwm2m_object_client_access_control_get();
    ...
    lwm2m_object_client_instance_create(obj_list[5], 0, &client_args);
...
int lwm2m_cli_cmd(int argc, char **argv)
{
    ...
    if (IS_ACTIVE(CONFIG_LWM2M_CLIENT_C2C) && !strcmp(argv[1], "read")) {
        if (argc != 4) {
...
read_usage_error:
    ...
    if (IS_ACTIVE(CONFIG_LWM2M_CLIENT_C2C) && !strcmp(argv[1], "obs")) {
        if (argc != 4) {
...
obs_usage_error:
    ...
    if (IS_ACTIVE(CONFIG_LwM2M_CLIENT_THIRD_PARTY_AUTH) && !strcmp(argv[1], "auth")) {
        if (argc != 7 && argc != 8) {
            printf("%d\n", argc);
...
auth_usage_error:
    ...
    if (IS_ACTIVE(CONFIG_LWM2M_CLIENT_C2C) && !strcmp(argv[1], "close")) {
        if (argc != 3) {
...
help_error:
    ...
    if (IS_ACTIVE(CONFIG_LWM2M_CLIENT_C2C)) {
        ...
        if (IS_ACTIVE(LwM2M_CLIENT_THIRD_PARTY_AUTH)) {
            printf("|auth");
        }
    }
