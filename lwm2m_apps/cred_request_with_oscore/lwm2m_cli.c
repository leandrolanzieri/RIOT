...
void lwm2m_cli_init(void)
{
    ...
    lwm2m_object_access_control_instance_create(obj, acc_ctrl_inst, &acc_args);

    acl_args.access = LWM2M_ACCESS_CONTROL_CREATE;
    acl_args.res_inst_id = CONFIG_LWM2M_SERVER_SHORT_ID;
    lwm2m_object_access_control_add(obj, acc_ctrl_inst, &acl_args);
    obj_list[obj_count++] = obj;
    ...
    if (obj_count > OBJ_COUNT) {
        puts("ERROR too many objects were created");
    }
}
